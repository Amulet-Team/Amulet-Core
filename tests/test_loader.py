...
class DefinitionBasedLoaderTestCase(unittest.TestCase):
    ...
    def test_identify(self):
        numerical_java_format = load_format(
            get_world_path(worlds_src.java_vanilla_1_12_2)
        )
        self.assertIsInstance(numerical_java_format, AnvilFormat)

        blockstate_java_format = load_format(
            get_world_path(worlds_src.java_vanilla_1_13)
        )
        self.assertIsInstance(blockstate_java_format, AnvilFormat)

    def test_loading(self):
        numerical_java_world = load_level(
            create_temp_world(worlds_src.java_vanilla_1_12_2)
        )
        ...
        blocksate_java_world = load_level(
            create_temp_world(worlds_src.java_vanilla_1_13)
        )
