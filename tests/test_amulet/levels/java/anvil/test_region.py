        ...
        with TemporaryDirectory() as tempdir:
            ...
            for region_file_path in glob.glob(
                ...
                with self.subTest(rel_path):
                    ...
                    original_chunk_data = {
                        (cx, cz): region.get_data(cx, cz)
                        for cx, cz in region.all_coords()
                    }

                    for (cx, cz), nbt_file in original_chunk_data.items():
                        path = os.path.join(
                            r"C:\Users\james_000\AppData\Local\Temp\chunk_data",
                            f"{rel_path}.{cx}.{cz}.bin",
                        )
                    ...
                    if original_file_size >= 0x2000:
                        self.assertLessEqual(
                            os.stat(region.path).st_size, original_file_size
                        )
