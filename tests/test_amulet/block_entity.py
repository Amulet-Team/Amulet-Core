...
def get_test_block_entity() -> BlockEntity:
    return BlockEntity(JavaGameVersion(3578), "namespace", "basename", NamedTag(
        CompoundTag({
            "int": IntTag(1),
            "str": StringTag("hi")
        })
...
def get_test_block_entity_variants() -> tuple[BlockEntity, ...]:
    return (
        BlockEntity(JavaGameVersion(3579), "namespace", "basename", NamedTag(
            CompoundTag({
                "int": IntTag(1),
                "str": StringTag("hi")
            })
        )),
        BlockEntity(JavaGameVersion(3578), "namespace1", "basename", NamedTag(
            CompoundTag({
                "int": IntTag(1),
                "str": StringTag("hi")
            })
        )),
        BlockEntity(JavaGameVersion(3578), "namespace", "basename1", NamedTag(
            CompoundTag({
                "int": IntTag(1),
                "str": StringTag("hi")
            })
        )),
        BlockEntity(JavaGameVersion(3578), "namespace", "basename", NamedTag(
            CompoundTag({
                "int": IntTag(0),
                "str": StringTag("hi")
            })
...
class BlockEntityTestCase(unittest.TestCase):
    def test_construct(self):
        ...
        self.assertEqual(JavaGameVersion(3578), block_entity.version)
