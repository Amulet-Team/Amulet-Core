...
from amulet.core.chunk.component import IndexArray3D, SectionArrayMap
import faulthandler
import gc
...
class IndexArray3DTestCase(unittest.TestCase):
    ...
    def test_construct_shape_value(self) -> None:
        ...
        numpy.testing.assert_array_equal(
            numpy.full((16, 16, 16), 5, dtype=numpy.uint32), numpy.asarray(array)
        )
    ...
    def test_construct_self(self) -> None:
        ...
        numpy.testing.assert_array_equal(
            numpy.full((16, 16, 16), 5), numpy.asarray(array2)
        )
    ...
    def test_np(self) -> None:
        ...
        array = IndexArray3D((16, 16, 16), 5)
        numpy.testing.assert_array_equal(
            numpy.full((16, 16, 16), 5), numpy.asarray(array)
        )
        numpy.asarray(array)[:] = 10
        numpy.testing.assert_array_equal(
            numpy.full((16, 16, 16), 10), numpy.asarray(array)
        )
...
class SectionArrayMapTestCase(unittest.TestCase):
    ...
    def test_construct_array(self) -> None:
        ...
        self.assertEqual((16, 16, 16), sections.array_shape)
        numpy.testing.assert_array_equal(
            numpy.zeros((16, 16, 16)), sections.default_array
        )

    def test_construct_np(self) -> None:
        sections = SectionArrayMap(
            (16, 16, 16), numpy.arange(16**3, dtype=numpy.uint32).reshape((16, 16, 16))
        )
        self.assertEqual((16, 16, 16), sections.array_shape)
        numpy.testing.assert_array_equal(
            numpy.arange(16**3, dtype=numpy.uint32).reshape((16, 16, 16)),
            sections.default_array,
        )

    def test_construct_errors(self) -> None:
        with self.assertRaises(ValueError):
            SectionArrayMap(
                (16, 16, 16), numpy.arange(8**3, dtype=numpy.uint32).reshape((8, 8, 8))
            )
        with self.assertRaises(ValueError):
    ...
    def test_default_array(self) -> None:
        ...
        sections.default_array = numpy.arange(16**3, dtype=numpy.uint32).reshape(
            (16, 16, 16)
        )
        numpy.testing.assert_array_equal(
            numpy.arange(16**3, dtype=numpy.uint32).reshape((16, 16, 16)),
    ...
    def test_populate(self) -> None:
        ...
        numpy.testing.assert_array_equal(
            numpy.ones((16, 16, 16), dtype=numpy.uint32), sections[0]
        )
        ...
        sections.populate(1)
        numpy.testing.assert_array_equal(
            numpy.arange(16**3, dtype=numpy.uint32).reshape((16, 16, 16)), sections[1]
        )
        sections.populate(-1)
        numpy.testing.assert_array_equal(
            numpy.arange(16**3, dtype=numpy.uint32).reshape((16, 16, 16)), sections[-1]
    ...
    def test_setitem(self) -> None:
        ...
        numpy.testing.assert_array_equal(
            numpy.full((16, 16, 16), 5, dtype=numpy.uint32), sections[0]
        )

        sections[1] = IndexArray3D((16, 16, 16), 10)
        numpy.testing.assert_array_equal(
            numpy.full((16, 16, 16), 10, dtype=numpy.uint32), sections[1]
        )
    ...
    def test_values(self) -> None:
        ...
        for v in sections.values():
            ...
            numpy.testing.assert_array_equal(
                numpy.ones((16, 16, 16), dtype=numpy.uint32), v
            )
