        ...
        if mcstructure["format_version"].value == 1:
            ...
            for location, block_entity in block_entities.items():
                if all(key in block_entity for key in "xyz"):
                    x, y, z = block_entity["x"].value, block_entity["y"].value, block_entity["z"].value
                    cx, cz = x >> 4, z >> 4
...
class MCStructureWriter:
    def __init__(
        ...
        self._blocks = numpy.zeros(selection.shape, dtype=numpy.uint32)  # only 12 bits are actually used at most
    ...
    def close(self):
        ...
        block_indices = numpy.array(block_palette_indices, dtype=numpy.int32)[self._blocks].T

        self._data["structure"] = amulet_nbt.TAG_Compound({
            "block_indices": amulet_nbt.TAG_List([  # a list of tag ints that index into the palette. One list per block layer
                amulet_nbt.TAG_List([
