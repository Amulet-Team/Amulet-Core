...
Updated and modified by Podshot for the Amulet Map Editor and for Python 3.7
"""
...
cdef str load_string(load_ctx ctx):
...
IF UNICODE_NAMES:
    cdef str load_name(load_ctx ctx):
        """
        Like load_string, but caches the str object in u_cache to save memory
        """
        cdef unsigned short * ptr = <unsigned short *> read(ctx, 2)
        cdef unsigned short length = ptr[0]
        swab(&length, 2)
        b = read(ctx, length)
        IF UNICODE_CACHE:
            s = PyUnicode_DecodeUTF8(b, length, "strict")
            u = u_cache.get(s)
            if u is None:
